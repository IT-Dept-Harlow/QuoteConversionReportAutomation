    ...
    public class ExcelCopyData
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                using (var destinationPackage = new ExcelPackage(new FileInfo(tempFilePath)))
                {
                    ...
                    if (reportType == ReportType.Daily5Day1k)
                    {
                        ...
                        Logger.LogInfo($"'{destinationDataSheetName}' sheet filtered for report type {reportType} with threshold {filterThreshold}.");
                        cancellationToken.ThrowIfCancellationRequested();
                    }
                ...
                await RenameFileWithRetryAsync(tempFilePath, finalFilePath, cancellationToken); // updated to remove progress parameter
                ...
                _statusManager.Post("Excel processing complete.", MessageType.Success); // updated progress reporting method
            ...
            catch (OperationCanceledException)
            {
                ...
                _statusManager.Post("Operation cancelled.", MessageType.Warning); // updated to use status manager
                return null;
            }
            catch (FileNotFoundException fnfEx)
            {
                ...
                _statusManager.Post($"Error: Required file not found - {Path.GetFileName(fnfEx.FileName)}", MessageType.Error); // updated to use status manager
                return null;
            }
            catch (InvalidOperationException opEx) // Catch specific operational errors.
            {
                ...
                _statusManager.Post($"Error: {opEx.Message}", MessageType.Error); // updated to use status manager
                return null;
            }
            catch (Exception ex) // Catch-all for other unexpected errors.
            {
                ...
                _statusManager.Post($"Error: {ex.Message}", MessageType.Error); // updated to use status manager
                return null;
            }
            finally
            {
            ...
            await Task.Run(() =>
            {
                ...
                for (int r = initialRowCount; r >= 2; r--) // Start from last row, down to row 2.
                {
                    ...
                    if ((initialRowCount - r) % 100 == 0)
                    {
                        ...
                        _statusManager.Post($"Filtering 'DATA' sheet... {processedRows}/{totalDataRows}", MessageType.InProgress); // Updated to use status manager
                    }
                }
                ...
                _statusManager.Post($"Filtering 'DATA' sheet complete.", MessageType.Success); // Updated to use status manager
            ...
            await Task.Run(() =>
            {
                ...
                for (int r = initialRowCount; r >= customerDataStartRow; r--)
                {
                    ...
                    if ((initialRowCount - r) % 20 == 0)
                    {
                        ...
                        _statusManager.Post($"Filtering Analysis sheet (zero estimates)... {processedIteration}/{totalRowsToProcess}", MessageType.InProgress); // Updated to use status manager
                    }
                }
                ...
                _statusManager.Post("Filtering Analysis sheet (zero estimates) complete.", MessageType.Success); // Updated to use status manager
            }, cancellationToken);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (reportType == ReportType.Daily5Day1k)
            {
                _statusManager.Post("Filtering Analysis sheet for zero estimates/values...", MessageType.InProgress);
                await FilterAnalysisSheetForZeroEstimatesAsync(package, targetAnalysisSheetName, cancellationToken);
                Logger.LogInfo($"Analysis sheet '{targetAnalysisSheetName}' filtered for zero estimates/values for report type {ReportType.Daily5Day1k}.");
                cancellationToken.ThrowIfCancellationRequested();
            }
            ...
            if (reportType == ReportType.Weekly)
            {
                ...
                await CopyAnalysisDataToPowerBIReportAsync(package, targetAnalysisSheetName, powerBiSheetName, reportType, originalSourceFilePath, reportDate, cancellationToken);
             ...
             CancellationToken cancellationToken)
        {
            ...
            _statusManager.Post($"Extracted {uniqueCustomers.Count} unique customers.", MessageType.InProgress);
        ...
        private async Task CopyAnalysisDataToPowerBIReportAsync(
            ...
            ReportType reportType,                 // For context, mainly to generate a source filename.
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (string.IsNullOrEmpty(destinationPowerBiFilePath))
            {
                ...
                _statusManager.Post("Error: Central Power BI report path invalid.", MessageType.Error, TimeSpan.FromSeconds(5));
                return;
            }
            if (!File.Exists(destinationPowerBiFilePath))
            {
                ...
                _statusManager.Post("Error: Central Power BI report file not found.", MessageType.Error, TimeSpan.FromSeconds(5));
                return;
            }
                ...
                await Task.Run(() =>
                {
                    ...
                    for (int sourceRow = startDataRowInAnalysis; sourceRow <= sourceAnalysisRowCount; sourceRow++)
                    {
                        ...
                        if ((sourceRow - startDataRowInAnalysis + 1) % 50 == 0 && sourceAnalysisRowCount > startDataRowInAnalysis)
                        {
                            ...
                            _statusManager.Post($"Copying to Power BI source... {Math.Min(100, percent)}%", MessageType.Info); // Base progress at 75%
                        }
                    }
                    ...
                    if (sourceAnalysisRowCount >= startDataRowInAnalysis)
                        _statusManager.Post("Copying to Power BI source complete.", MessageType.Info); // Progress after this step.
                }, cancellationToken);
            ...
            catch (OperationCanceledException)
            {
                ...
                _statusManager.Post("Cancelled copy to Power BI source.", MessageType.Warning, TimeSpan.FromSeconds(5));
            ...
            catch (IOException ioEx) // Catch common IO errors (file locked, etc.)
            {
                ...
                _statusManager.Post($"Error (Power BI): {ioEx.Message}", MessageType.Error, TimeSpan.FromSeconds(5));
            }
            catch (Exception ex) // Catch other unexpected errors.
            {
                ...
                _statusManager.Post($"Error (Power BI): {ex.Message}", MessageType.Error, TimeSpan.FromSeconds(5));
            }
        ...
        private string GenerateFinalFileName(ReportType reportType, DateTime reportDate, DateTime runTimestamp)
        {
            ...
            switch (reportType)
            {
                ...
                case ReportType.Quarterly:
                    fileName = $"Estimate Success Rate {ReportHelper.GetQuarterString(reportDate)}.xlsx"; // e.g., "Estimate Success Rate Q1 2023.xlsx"
                    break;
                case ReportType.Annual:
                    fileName = $"Estimate Success Rate FY {ReportHelper.GetFinancialYearStartCalendarYear(reportDate, _configuration)}-{ReportHelper.GetFinancialYearStartCalendarYear(reportDate, _configuration) + 1}.xlsx";
                    break;
